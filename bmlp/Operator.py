...
def pinv(pred):
    return init_predicate(pred.get_matrix().T, f"({pred.get_name()}.T)")


# Create the negation of M using sparse matrix
def pneg(pred):
    return init_predicate(matrix_negate(pred.get_matrix()), f"(~ {pred.get_name()})")


# Create the conjunction of two predicate objects if they share the same variables
def pconj(pred1, pred2):
    return init_predicate(pred1.get_matrix() * pred2.get_matrix(), f"({pred1.get_name()}, {pred2.get_name()})")
...
def pchain_1(pred1, pred2):
    return init_predicate(pred1.get_matrix() @ pred2.get_matrix(), f"(inv <- {pred1.get_name()}, {pred2.get_name()})")


def pchain_2(pred1, pred2):
    return init_predicate(pred2.get_matrix() @ pred1.get_matrix(), f"(inv <- {pred2.get_name()}, {pred1.get_name()})")


def pchain_self(pred):
    return init_predicate(pred.get_matrix() @ pred.get_matrix(), f"(inv <- {pred.get_name()}, {pred.get_name()})")
